package com.jigsaw.data.mapper
...
object DataMappers {
    ...
    fun toEntity(game: JigsawGame): GameEntity = GameEntity(
        ...
        levelJson = gson.toJson(
            LevelJson(
                gridSize = game.level.gridSize,
                initialTiles = game.level.initialTiles,
                puzzleIndex = game.level.puzzleIndex
            )
        ),
    ...
    fun fromEntity(entity: GameEntity): JigsawGame {
        val levelJson = gson.fromJson(entity.levelJson, LevelJson::class.java)
        val tiles: List<Int> = gson.fromJson(entity.tubeStateJson, intListType)
        val level = JigsawLevel(
            id = entity.id,
            seed = entity.seed,
            levelNumber = entity.levelNumber,
            difficulty = Difficulty.valueOf(entity.difficulty),
            gridSize = levelJson.gridSize,
            initialTiles = levelJson.initialTiles,
            puzzleIndex = levelJson.puzzleIndex,
