package com.jigsaw.data.repository
...
import com.jigsaw.data.mapper.DataMappers
...
import com.jigsaw.engine.PuzzleProfileEngine
import com.jigsaw.engine.JigsawEngine
import com.jigsaw.engine.JigsawGenerator
import com.jigsaw.engine.TutorialLevels
import com.jigsaw.util.DateUtils
...
class GameRepositoryImpl @Inject constructor(
    ...
    override suspend fun createNewGame(difficulty: Difficulty, levelNumber: Int): JigsawGame {
        ...
        val seed = JigsawGenerator.seedFromLevelNumber(levelNumber, difficulty)
        val level = JigsawGenerator.generate(seed, levelNumber, difficulty, generationProfile)
        return JigsawEngine.createInitialGame(level)
    }
    ...
    override suspend fun createGameFromSeed(
        ...
        val level = JigsawGenerator.generate(seed, levelNumber, difficulty)
        return JigsawEngine.createInitialGame(level)
    }

    override suspend fun createTutorialGame(tutorialIndex: Int): JigsawGame? {
        val level = TutorialLevels.getTutorialLevel(tutorialIndex) ?: return null
        return JigsawEngine.createInitialGame(level)
    }

    override suspend fun createEndlessGame(wave: Int): JigsawGame {
        val seed = wave.toLong() * 7919L + System.currentTimeMillis() % 1000
        val level = JigsawGenerator.generate(seed, wave, Difficulty.ENDLESS)
        return JigsawEngine.createInitialGame(level)
    }

    override suspend fun saveGame(game: JigsawGame): Long {
    ...
    override suspend fun getGame(gameId: Long): JigsawGame? =
        gameDao.getById(gameId)?.let { DataMappers.fromEntity(it) }

    override suspend fun getInProgressGame(): JigsawGame? =
        gameDao.getInProgress()?.let { DataMappers.fromEntity(it) }

    override fun observeInProgressGame(): Flow<JigsawGame?> =
        gameDao.observeInProgress().map { it?.let { e -> DataMappers.fromEntity(e) } }

    override suspend fun completeGame(game: JigsawGame): JigsawGame {
    ...
    override suspend fun getLevel(seed: Long, levelNumber: Int, difficulty: Difficulty): JigsawLevel =
        JigsawGenerator.generate(seed, levelNumber, difficulty)
}
...
class ChallengeRepositoryImpl @Inject constructor(
    ...
    override suspend fun getChallengeGame(record: ChallengeRecord): JigsawGame {
        val levelNumber = record.key.hashCode().and(0x7FFFFFFF) % 5000 + 1
        val level = JigsawGenerator.generateForChallenge(
...
class ProgressionRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateStatsAfterGame(game: JigsawGame) {
    ...
    private suspend fun updatePuzzleProfile(game: JigsawGame) {
    ...
    private fun updateFastest(current: Long, game: JigsawGame, difficulty: Difficulty): Long {
...
class PreferencesRepositoryImpl @Inject constructor(
    private val dataStore: com.jigsaw.data.local.PreferencesDataStore
) : com.jigsaw.domain.repository.PreferencesRepository {

    override fun getUserPreferences() = dataStore.preferencesFlow

    override suspend fun updatePreferences(
        transform: (com.jigsaw.domain.model.UserPreferences) -> com.jigsaw.domain.model.UserPreferences
